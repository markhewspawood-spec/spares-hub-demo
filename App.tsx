...
/**
 * SparesHub - Single-file App.tsx
 * - Simple internal router (no libraries)
 * - Landing hero with overlay CTA buttons
 * - Footer with legal + contact + sitemap-style links
 *
 * Asset required:
 * - public/landing.png
 */
...
function App() {
  ...
  // Keep route in sync with browser back/forward
  useEffect(() => {
          ...
          <div style={styles.footerGrid}>
            ...
            <div>
              ...
              <div style={styles.footerText}>
                For partnership enquiries, press, or support:
              </div>
    ...
    >
      <div style={styles.card}>
        ...
        <div style={styles.heroMicroDark}>
          This is an MVP placeholder. Next we’ll add real authentication and saving.
        </div>
      </div>
    </PageShell>
  );
  ...
  return (
    ...
    >
      <div style={styles.cardNarrow}>
        ...
        <button
          style={styles.ctaPrimarySolid}
          onClick={() => alert("MVP: Sign-in placeholder (no real auth yet).")}
        >
          Sign In
        </button>
        ...
        <div style={{ height: 12 }} />
        <div style={styles.heroMicroDark}>
          No account yet? <InlineLink onClick={() => onNav("/contact")}>Contact us</InlineLink>{" "}
          for early access.
        </div>
      </div>
    </PageShell>
  ...
  return (
    <PageShell title="Contact" subtitle="Get in touch with SparesHub.">
      <div style={styles.cardNarrow}>
        ...
        <button
          style={styles.ctaPrimarySolid}
          onClick={() => alert("MVP: Message send placeholder.")}
        >
          Send
        </button>
      </div>
    ...
    <PageShell title="Help / FAQ" subtitle="Common questions (MVP placeholder).">
      <div style={styles.card}>
        ...
        <Faq
          ...
          a="Use Search and filter by make, model, and category. We’ll add seller verification and fitment matching next."
        />
        <Faq
          q="How do I sell a part?"
          a="Use Sell to create a listing. In the full app, you’ll upload photos, set shipping, and manage messages."
        />
        <Faq
          q="Is SparesHub secure?"
          a="This is an MVP build. Next we’ll add authentication, secure storage, and audit trails."
        />
      </div>
    </PageShell>
  );
...
function LegalPage({
  ...
  title: string;
  kind: "terms" | "privacy" | "cookies" | "disclaimer";
}) {
  const content = getLegalCopy(kind);
  return (
...
const styles: Record<string, React.CSSProperties> = {
  ...
  heroOverlay: {
    ...
    inset: 0,
    background:
      "linear-gradient(to bottom, rgba(0,0,0,0.08), rgba(0,0,0,0.38) 55%, rgba(0,0,0,0.62))",
  },
  heroContent: {
    position: "absolute",
    left: "50%",
...
/* Mobile: stack CTA buttons and simplify nav spacing (no CSS file; just inline) */
(function injectResponsiveStyleOnce() {
...
/* Apply classnames to enable the responsive overrides */
(function patchClassnamesOnce() {
  // No-op: this is here so you can easily extend class-based rules later if desired.
})();
